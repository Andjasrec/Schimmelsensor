...
export default {
  components: { 
    ...
    data() {
      return {
        Luftfeuchtigkeit:"verbindung wird aufgebaut",
        Temperatur:"vebindung wird aufgebaut" 
      }
    },
    mounted() {
      axios.get('https://schimmelsensor_backend.ds.ava.hfg.design/api/latest')
      .then(response => {
        //this.data = response.data.Luftfeuchtigkeit))
        console.log(response);
        this.Luftfeuchtigkeit = response.data.Luftfeuchtigkeit
...
width: 113px;
height: 36px;
left: calc(50% - 113px/2 + 2px);
top: calc(50% - 36px/2 - 328px);
/* UI - 30 Semi */
font-family: Inter;
font-style: normal;
font-weight: 600;
...
width: 343px;
height: 36px;
left: calc(50% - 343px/2 + 3px);
top: calc(50% - 36px/2 + 248px);
font-family: Inter;
font-style: normal;
font-weight: 600;
font-size: 21px;
